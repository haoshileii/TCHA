...
from models.lossesHCLcluster import HCL_loss
# from models.lossesHCLcluster_XR_outdtw import HCL_loss
from utils import take_per_row, split_with_nan, centerize_vary_length_series, torch_pad_nan
import math
import random
from dataaugmentations import data_augmentation, data_augmentationWR
    ...
    def fit(self, train_data, train_labels, n_epochs=None, n_iters=None, verbose=False):
        ...
        while True:
            ...
            for id, batch in enumerate(train_loader,1):
                ...
                x = x.to(self.device)
                ts_l = x.size(1)
                crop_l = np.random.randint(int(ts_l/2), high=ts_l+1)
                crop_left = np.random.randint(ts_l - crop_l + 1)
                crop_right = crop_left + crop_l
                print(crop_l)
                nm_train = 3
                x_strong, x_weak = data_augmentationWR(x_aug)
                x_weak = torch.from_numpy(x_weak)
                x_weak = x_weak.to(self.device)
                x_strong = x_strong.to(self.device)
                x = x.to(torch.float32)
                x_weak = x_weak.to(torch.float32)
                x_strong = x_strong.to(torch.float32)
                out1 = self._net(x)
                out2 = self._net(x_strong)
                out3 = self._net(x_weak)
    ...
    def encode(self, data, data_labels, mask=None, encoding_window=None, casual=False, sliding_length=None, sliding_padding=0, batch_size=None):
        ...
        with torch.no_grad():
            ...
            for batch in loader:
                ...
                output.append(out)
                labelput.append(x_label)
                
            output = torch.cat(output, dim=0)
            labelput = torch.cat(labelput, dim=0)
            
        self.net.train(org_training)
        return output.numpy(), labelput.numpy()
